...
test_that("contour works", {
  ...
  expect_equal(nrow(iso),6)
  expect_equal(nrow(tanaka_contour(x = ras, mask = as(com, "Spatial"))),7)
  expect_silent(plot(st_geometry(iso)))
})
...
fufun <- function(){
  for (i in list("bottomleft", "topleft", "topright", "bottomright",
    ...
    tanaka(x = ras, legend.pos =i, breaks = c(80,200,400))
  }
}
locator()
